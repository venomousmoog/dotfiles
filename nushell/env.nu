# env.nu -- Environment setup for nushell dotfiles
# Runs before config.nu on every shell startup
...
# Resolve dotfiles root (parent of nushell/ dir)
# Priority: 1) already set by user, 2) FILE_PWD (startup sourcing),
# 3) resolve $nu.env-path through symlinks (works when env.nu is symlinked)
# For interactive testing: set $env.DOTFILES_PATH first, then source this file
$env.DOTFILES_PATH = (if 'DOTFILES_PATH' in $env {
    $env.DOTFILES_PATH
} else if 'FILE_PWD' in $env {
    $env.FILE_PWD | path expand | path dirname
} else {
    $nu.env-path | path expand | path dirname | path dirname
})

# NU_LIB_DIRS -- tells nushell where to search for source/use files
# Includes dotfiles nushell dir so config.nu can "source listing.nu" etc.
$env.NU_LIB_DIRS = [
...
# zoxide init (generate cache file for sourcing in config.nu)
mkdir ~/.cache/nushell
