...
function Format-GitBranchName {
    ...
    $res = $BranchName.Replace($env:USERNAME, "~")
    if (($s.BranchNameLimit -gt 0) -and ($res.Length -gt $s.BranchNameLimit))
    {
        $res = "{0}{1}" -f $res.Substring(0, $s.BranchNameLimit), $s.TruncatedBranchSuffix
    }
